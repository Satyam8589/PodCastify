...
const AdminUploadPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = contentType === "podcast" ? "/api/podcasts" : "/api/blog";
      ...
      if (responseText) {
        try {
          ...
          setSubmitStatus({
            type: "success",
            message: responseText || `${contentType === "podcast" ? "Podcast" : "Blog post"} uploaded successfully!`
          });
          resetForm();
          return;
        }
      ...
      if (responseData) {
        if (responseData.success !== false) {
          setSubmitStatus({
            type: "success",
            message: responseData.message || `${contentType === "podcast" ? "Podcast" : "Blog post"} uploaded successfully!`
          });
          resetForm();
        } else {
          throw new Error(responseData.error || responseData.message || `Failed to upload ${contentType}`);
        }
      } else {
        // Empty successful response
        setSubmitStatus({
          type: "success",
          message: `${contentType === "podcast" ? "Podcast" : "Blog post"} uploaded successfully!`
        });
        resetForm();
      }
